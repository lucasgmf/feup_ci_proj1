...
int readHoldingRegisters(int socketfd,
                         uint16_t id,
    ...
    if (startingAddress + quantity > MODBUS_ADDRESS_MAX) {
        PRINT("[App][RHR] - Error: Number of registers to read exceeds maximum\n");
        return -1;
    }
    if (dataToRead == NULL) {
    ...
    int bytesReceived = sendModbusRequest(socketfd, id, apdu, apduLen);
    if (bytesReceived < 0) {
 ...
 * @return int
 */
int writeMultipleRegisters(int socketfd,
                           uint16_t id,
                           uint16_t startingAddress,
                           uint16_t quantity,
                           uint16_t* data) {
    ...
    if (startingAddress + quantity > MODBUS_ADDRESS_MAX) {
        PRINT("[App][WMR] - Error: Number of registers to read exceeds maximum\n");
        return -1;
    }
